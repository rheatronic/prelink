...
void StatusStream::SetMode(std::string &lBuffer, bool pMode, char* pItem)
{
...
void StatusStream::ParseBuffer(PrevizionUpdate& update, const char* pBuffer, int pNumberOfBytesRead)
{
	// Append new chars to buffer
	mBuffer.append(pBuffer, pNumberOfBytesRead);
	...
	if( lStart != std::string::npos )
	{
		...
		{
			// parse the section
			ParseRecord( update, 0, lStop + strlen(END_TOKEN) );
...
void StatusStream::ParseRecord( PrevizionUpdate& update, std::string::size_type pStart, std::string::size_type pStop )
{
 ...
 <status>
	...
	*/
	...
	try {
		...
		// if throw should delete this
		PrevizionFrame* lFrame = new PrevizionFrame();

		// Should be time and rate attrs
		for(std::size_t i=0; i<lAttrs.size(); i++)
			if( lAttrs[i].first == "timecode" )
			{
				Read(lAttrs[i].second, lFrame->mTime.hour, lFrame->mTime.minute, lFrame->mTime.second, lFrame->mTime.frame);
				lFrame->mHasTime = true;
			}
			else if( lAttrs[i].first == "rate" )
			{
				Read(lAttrs[i].second, lFrame->mTime.rate);
			}
			else
				throw BadSyntax();
		...
		// XXX if throw should delete this
		CameraStatus* lCamera = new CameraStatus();
		...
		lFrame->mStates.push_back(lCamera);
		...
		update.push_back(lFrame);

		// XXX frame allocation guard ends here
